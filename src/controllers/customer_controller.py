"""Customer controller — CRUD, search, pagination, CSV export."""
import csv
import io
...
def index():
    ...
    return render_template('Customers.html',
                           customers=customers, stats=CustomerModel.get_stats(),
...
def add():
    ...
    CustomerModel.create(full_name=full_name, email=email,
                         phone=phone or None, id_card=id_card or None,
...
def export():
    ...
    for c in customers:
        writer.writerow([
            ...
            c['total_spent'], c['total_bookings'], c['status'], c['created_at']
        ])
    output.seek(0)
